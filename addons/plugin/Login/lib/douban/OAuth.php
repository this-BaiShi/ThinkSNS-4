  ...
  {/*{{{*/
    ...
    if ($parameters) {
      ...
      if (@substr($request_headers['Authorization'], 0, 6) == "OAuth ") {
          ...
          $req = new doubanOAuthRequest($http_method, $http_url, $req_parameters);
      }
    }
  ...
  {/*{{{*/
    $pattern = '/(([-_a-z]*)=("([^"]*)"|([^,]*)),?)/';
      ...
      while (preg_match($pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
          ...
          $params[$header_name] = doubanOAuthUtil::urldecode_rfc3986($header_content);
...
class doubanOAuthServer
{
  ...
  {/*{{{*/
    ...
    $now = time();
      if ($now - $timestamp > $this->timestamp_threshold) {
          throw new doubanOAuthException("Expired timestamp, yours $timestamp, ours $now");
      }
  }/*}}}*/
  ...
  {/*{{{*/
    ...
    $found = $this->data_store->lookup_nonce($consumer, $token, $nonce, $timestamp);
      if ($found) {
          throw new doubanOAuthException("Nonce already used: $nonce");
      }
  }/*}}}*/
}/*}}}*/

class doubanOAuthDataStore
{
    /*{{{*/
...
class doubanOAuthUtil
{
    /*{{{*/
  public static function urlencode_rfc3986($input)
  {/*{{{*/
  if (is_array($input)) {
      return array_map(array('doubanOAuthUtil', 'urlencode_rfc3986'), $input);
